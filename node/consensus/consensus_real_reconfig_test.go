...
func TestConsensusWithRealConfigUpdate(t *testing.T) {
	...
	t.Run("config update with leader consenter removal", func(t *testing.T) {
		...
		// send another simple request
		lastBlockNumber++
		sendSimpleRequest(t, consensusNodes, privateKey, privateKey2, 1, 2, configSeq, lastBlockNumber, "key does not exist")
		sendSimpleRequest(t, consensusNodes, privateKey2, privateKey2, 2, 2, configSeq, lastBlockNumber, "")
	})
...
func sendSimpleRequest(t *testing.T, consensusNodes []*consensus_node.Consensus, privateKey, primaryPrivateKey *ecdsa.PrivateKey, batcherID, primaryID types.PartyID, configSeqToSend types.ConfigSequence, expectedBlockNumber uint64, expectedError string) {
